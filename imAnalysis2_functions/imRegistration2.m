...
[stimLengthQ] = getUserInput(userInput,'Are all stim trains the same length in time?');
if stimLengthQ == 1    
    stimLengthsQ = 1; 
    [stimTimes] = getUserInput(userInput,'Stim Time Lengths (sec)');   
elseif stimLengthQ == 0
    [stimLengthsQ] = getUserInput(userInput,'How many different stimulus time lengths are there?');
    [stimTimes] = getUserInput(userInput,'Stim Time Lengths (sec)');  
end 
