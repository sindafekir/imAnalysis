...
for mouse = 1:mouseNum
    for ccell = 1%:length(terminals{mouse})  
        ...
        Ipc1 = reshape(Itransformed,size(I,1),size(I,2),size(I,3));
        
        fig1 = Ipc1(:,:,1);
        % normalize
        fig1Max = max(max(Ipc1(:,:,1)));
        norm1 = fig1./fig1Max;
        % multiply I by fig1 filter 
        fig1Mask = repmat(norm1,1,1,27);
        fig1filter = fig1Mask.*I;
        implay(fig1filter)
