...
% 1) CHANGE SPIKE FINDING CODE: NEED TO SORT PEAKS INTO TRIALS BY TYPE 
% 2) NEED TO FIGURE OUT BUG IN TIMING CODE RELATED TO WINDOW AVERAGING 
...
if allCaROIplotQ == 1 
    ...
    for tType = 1:length(raster2{term})
        ...
        stimStartF = sec_before_stim_start;
        hold all 
        if tType == 1 || tType == 3
            stimStopF = stimStartF + 2;           
            Frames = ceil(length(raster2{term}{tType})/winFrames);    
            Frames_pre_stim_start = -((Frames-1)/2); 
            Frames_post_stim_start = (Frames-1)/2; 
            sec_TimeVals = floor((((Frames_pre_stim_start*winSec):2:(Frames_post_stim_start*winSec)))+1);
            FrameVals = floor(((1:2:Frames*winSec)-1)/winSec);            
        elseif tType == 2 || tType == 4       
            stimStopF = stimStartF + 20;            
            Frames = ceil(length(raster2{term}{tType})/winFrames);       
            Frames_pre_stim_start = -((Frames-1)/2); 
            Frames_post_stim_start = (Frames-1)/2; 
            sec_TimeVals = floor((((Frames_pre_stim_start*winSec):2:(Frames_post_stim_start*winSec)))+10);
            FrameVals = floor(((1:2:Frames*winSec)-1)/winSec);
        end 
        if tType == 1 || tType == 2
            plot([stimStartF stimStartF], [0 size(fullRaster{tType},1)], 'b','LineWidth',2)
...
for mouse = 1:mouseNum
    ...
    elseif tTypeQ == 1
        for per = 1:size(allCTraces{1}{CaROIs{1}(1)},2)
            for CAROI = 1:size(CaROIs{mouse},2)
                %remove rows full of 0s/Nans if there are any b = a(any(a,2),:)
                allCTraces{mouse}{CaROIs{mouse}(CAROI)}{per} = allCTraces{mouse}{CaROIs{mouse}(CAROI)}{per}(any(allCTraces{mouse}{CaROIs{mouse}(CAROI)}{per},2),:);
                for BBBroi = 1:size(allBTraces{mouse},2)
                    allBTraces{mouse}{BBBroi}{CaROIs{mouse}(CAROI)}{per} = allBTraces{mouse}{BBBroi}{CaROIs{mouse}(CAROI)}{per}(any(allBTraces{mouse}{BBBroi}{CaROIs{mouse}(CAROI)}{per},2),:);
                end 
                for VWroi = 1:size(allVTraces{mouse},2)
                    allVTraces{mouse}{VWroi}{CaROIs{mouse}(CAROI)}{per} = allVTraces{mouse}{VWroi}{CaROIs{mouse}(CAROI)}{per}(any(allVTraces{mouse}{VWroi}{CaROIs{mouse}(CAROI)}{per},2),:);
                end 
            end 
        end 
    end 
...
if tTypeQ == 1
    ...
    if firstTimeQ == 1
        allBTraces2 = allBTraces;
        allCTraces2 = allCTraces;
        allVTraces2 = allVTraces;
    end 
    per = input('Input 1 for blue light period. Input 2 for red light period. Input 3 for light off period. ');
...
for mouse = 1:mouseNum
    ...
    if BBBQ == 1
        for BBBroi = 1:BBBroiNum(mouse)
            closeBTraces{mouse}{BBBroi} = allBTraces{mouse}{BBBroi}(closeCaROIs{mouse});
            farBTraces{mouse}{BBBroi} = allBTraces{mouse}{BBBroi}(farCaROIs{mouse});
        end
    end 
    if VWQ == 1
        for VWroi = 1:VWroiNum(mouse)
...
% plot close Ca ROI and BBB data (all mice averaged)
Frames = minLen;
Frames_pre_stim_start = -((Frames-1)/2); 
Frames_post_stim_start = (Frames-1)/2; 
...
% plot close Ca ROI and VW data (all mice averaged) 
if VWQ == 1
    fig = figure;
    ax=gca;
    hold all
...
% plot far Ca ROI and BBB data (all mice averaged) 
if BBBQ == 1
    fig = figure;
    ax=gca;
    hold all
...
% plot far Ca ROI and VW data (all mice averaged) 
if VWQ == 1
    fig = figure;
    ax=gca;
    hold all
...
elseif tTypeQ == 1
    ...
    for groupNum = 1:numGroups
        for ccell = 1:length(terms{groupNum})            
            ...
            if BBBQ == 1
                for BBBroi = 1:length(sortedBdata{1})
                    AVSNBdataPeaks2{groupNum}{BBBroi}{terms{groupNum}(ccell)}{per} = (nanmean(allBTraces{groupNum}{BBBroi}{terms{groupNum}(ccell)}{per}));
                end 
            end 
...
%                         end 
            ...
            if BBBQ == 1
                for BBBroi = 1:length(sortedBdata{1})
                    AVSNBdataPeaks3{groupNum}{BBBroi}{per}(ccell,:) = nanmean(BTraces{groupNum}{BBBroi}{terms{groupNum}(ccell)}{per});
                end 
            end 
