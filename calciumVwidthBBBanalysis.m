...
% can generate fake/shuffled spikes and bootstrap 
%{
greenStacksOrigin = greenStacks;
redStacksOrigin = redStacks;
bootQ = input('Input 1 to bootstrap (x9) the data. Input 0 otherwise. ');
if bootQ == 0 
    bootNum = 1;
elseif bootQ == 1 
    bootNum = 9; 
...
for ccell = 1:length(terminals{mouse})
    ...
    % for scatter3
    unIdxVals = unique(idx{terminals{mouse}(ccell)}); minIdxVal = min(unIdxVals);
    idx{terminals{mouse}(ccell)}(idx{terminals{mouse}(ccell)}<0) = NaN;
    unIdxVals(unIdxVals<0) = NaN;
    ...
    for clust = 1:length(unIdxVals)
        ...
        [Crow, ~] = find(idx{terminals{mouse}(ccell)} == unIdxVals(clust)); 
