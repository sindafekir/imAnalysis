...
%     SEMdata = cell(1,length(nsCeta{1}));
        if isempty(nsCeta{terminals(ccell)}{tType}) == 0  
            ...
            if VWpQ == 1
                plot(AVvData{VWroi}{tType},'Color',[0.5 0 0],'LineWidth',3)
                patch([x fliplr(x)],[CI_vLow{VWroi}{tType} fliplr(CI_vHigh{VWroi}{tType})],[0.5 0 0],'EdgeColor','none')            
            end 
            if tType == 1 
...
if tTypeQ == 0 
    ...
    for ccell = 1:length(terminals)
        % plot 
        fig = figure;
        Frames = length(SNBdataPeaks{1}{BBBroi}{terminals(ccell)});
        Frames_pre_stim_start = -((Frames-1)/2); 
        Frames_post_stim_start = (Frames-1)/2; 
