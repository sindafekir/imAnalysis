function [sortedTrials,indices] = eventTriggeredAverages_STACKS(Tdata,state_start_f,FPS,indices,uniqueTrialData,uniqueTrialDataOcurr,userInput,numZplanes)
    ...
    for indGroup = 1:length(uniqueTrialDataOcurr)     
        ...
        if dataParseType == 0    
            ...
            for ind = 1:length(indices{indGroup})       
                ...
                elseif trial_end(ind) > length(Tdata)
                    indices{indGroup}(ind) = []; 
                end 
            end   
