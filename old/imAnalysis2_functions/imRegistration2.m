function [regStacks,userInput,UIr,state_start_f,state_end_f,vel_wheel_data,TrialTypes,HDFchart] = imRegistration2(userInput)
UIr = size(userInput,1)+1;
[volIm] = getUserInput(userInput,'Is this volume imaging data? Yes = 1. Not = 0.');
if volIm == 1
    [splitType] = getUserInput(userInput,'How must the data be split? Serial Split = 0. Alternating Split = 1.');
...
[channelOfInterest] = getUserInput(userInput,'What color channel is most relevant?'); 
...
cd(imAn2funcDir);
[HDFchart,state_start_f,state_end_f,~,vel_wheel_data,TrialTypes] = makeHDFchart_redBlueStim(state,framePeriod);
...
%         end              
        ...
        gVolStack5 = zeros(size(rVolStack,1),size(rVolStack,2),size(rVolStack,3)+size(rVolStack,3)-1,size(rVolStack,4));
%         rVolStack5 = zeros(size(rVolStack,1),size(rVolStack,2),size(gVolStack,3)+size(gVolStack,3)-1,size(gVolStack,4));
        for ind = 1:size(rVolStack,4)
            count = 1;
            for zplane = 1:size(rVolStack,3)+size(rVolStack,3)-1
                if rem(zplane,2) == 0
                    gVolStack5(:,:,zplane,ind) = mean(rVolStack(:,:,count-1:count,ind),3);
%                     rVolStack5(:,:,zplane,ind) = mean(rVolStack(:,:,count-1:count,ind),3);
                elseif rem(zplane,2) == 1 
                    gVolStack5(:,:,zplane,ind) = rVolStack(:,:,count,ind);
...
%         gVolStack5 = zeros(size(gVolStack,1),size(gVolStack,2),size(gVolStack,3)+size(gVolStack,3)-1,size(gVolStack,4));
        rVolStack5 = zeros(size(rVolStack,1),size(rVolStack,2),size(rVolStack,3)+size(rVolStack,3)-1,size(rVolStack,4));
        for ind = 1:size(rVolStack,4)
            count = 1;
            for zplane = 1:size(rVolStack,3)+size(rVolStack,3)-1
                if rem(zplane,2) == 0
...
%         rgVZstacks3 = cell(1,size(gZstacks,2));
        rrVZstacks3 = cell(1,size(rZstacks,2));
